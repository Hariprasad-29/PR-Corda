...
public class ConsultantController extends CommonController {
    ...
    private ResponseEntity sendPRRequest(@RequestBody PRBO prData) {
        ...
        try {
            FlowHandle<SignedTransaction> flowHandle = connector.getRPCops().startFlowDynamic(ConsultantInitiator.class,
                    new PRFlowData(prState, previousPRRequest, command));
    ...
    public ResponseEntity issueCash(@RequestParam("amount") Long amount, @RequestParam("currency") String currency) {
        ...
        try {
            final FlowHandle<AbstractCashFlow.Result> flowHandle = connector.getRPCops()
                    .startFlowDynamic(CashIssueFlow.class, issueRequest);
    ...
    @CrossOrigin
    @GetMapping("/")
    public ResponseEntity getPRRequestDetails(@RequestParam(value = "id", required = false) String id)
            throws Exception {
    ...
    /**
     *
     * @param command       is a parameter which creates a new Academic form request
     *                      (CREATE)
     * @param requestFormBO is a json object which we provide as an input to our
     *                      post api
     * @return It returns status whether new Academic form request has created or
     *         not
     */
    ...
    public ResponseEntity raiseBookingRequest(@PathVariable("command") String command,
        ...
        requestFormState = convertToRequestFormState(requestFormBO, consultantParty, wesParty, universityParty,
                RequestStatus.APPLICATION_SUBMITTED);

        try {
            FlowHandle<SignedTransaction> flowHandle = connector.getRPCops()
                    .startFlowDynamic(RequestFlowInitiator.class, new RequestFlowData(requestFormState,
                            previousrequestFormState, contractCommand));
            signedTransaction = flowHandle.getReturnValue().get();
            logger.info(String.format("signed Tx id: %s", signedTransaction.getId().toString()));
            return ResponseEntity
                    .ok("Student Transcript Request created with Txn Id: " + signedTransaction.getId().toString());
        } catch (InterruptedException | ExecutionException e) {
            logger.error(e.getMessage(), e.getCause());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
     ...
     * @param requestStatus is a parameter to change request status
     *                      (APPLICATION_SUBMITTED etc.)
    ...
    public ResponseEntity confirmTranscriptDetails(@PathVariable("requestId") String requestId,
        ...
        if (StringUtils.isEmpty(requestId)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("RequestId! is missing. Please enter a valid booking Id.");
        }

        contractCommand = new RequestFormContract.Commands.UPDATE();

        List<StateAndRef<RequestForm>> previousRequest = PRControllerHelper.getRequestFormStateFromLinearId(requestId,
                connector.getRPCops());

        if (previousRequest == null || previousRequest.isEmpty()) {
            throw new PRServerException(
                    "Request Id with id: " + requestId + " doesn't exist please verify and try again!");
        }

        if (!previousRequest.get(0).getState().getData().getRequestStatus()
                .equals(RequestStatus.ADDED_TRANSCRIPT_DETAILS)) {
            throw new IllegalArgumentException(
                    "Invalid request status, Expected Status : " + RequestStatus.ADDED_TRANSCRIPT_DETAILS);
        }

        RequestForm newRequestFormState = new RequestForm(previousRequest.get(0).getState().getData(),
                RequestStatus.CONFIRMED);

        try {
            FlowHandle<SignedTransaction> signedTransactionFlowHandle = connector.getRPCops().startFlowDynamic(
                    RequestFlowInitiator.class,
            ...
            return ResponseEntity.ok(
                    "Transcript Details confirmed successfully with Txn ID: " + signedTransaction.getId().toString());
        } catch (InterruptedException | ExecutionException e) {
            e.printStackTrace();
    ...
    @CrossOrigin
    @PostMapping("/redeem/{requestId}")
    private ResponseEntity redeeemToken(@RequestBody String tokenId,
            @PathVariable(value = "requestId", required = true) String requestId) {

        try {
            String result = connector.getRPCops().startFlowDynamic(QueryToken.class, tokenId).getReturnValue().get();
            return ResponseEntity.ok("Token is :" + result);
        } catch (InterruptedException | ExecutionException e) {
            logger.error(e.getMessage(), e.getCause());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
